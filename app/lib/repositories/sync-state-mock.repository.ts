...
import { mockSyncStateRepository } from '@/app/lib/db';
...
export class SyncStateRepository {
  async getSyncState(dataSource: string = 'public-data-portal') {
    return await mockSyncStateRepository.getSyncState();
  }

  async createSyncState(dataSource: string = 'public-data-portal') {
    const syncState = await mockSyncStateRepository.getSyncState();
